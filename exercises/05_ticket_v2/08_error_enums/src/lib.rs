...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        Ok(ticket) => ticket,
        Err(err) => match err {
            TicketNewError::EMPTY_TITLE => panic!("Title cannot be empty"),
            TicketNewError::SIZE_TITLE => panic!("Title cannot be longer than 50 bytes"),
            TicketNewError::SIZE_DESCRIPTION | TicketNewError::EMPTY_DESCRIPTION => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::EMPTY_TITLE);
        }
        if title.len() > 50 {
            return Err(TicketNewError::SIZE_TITLE);
        }
        if description.is_empty() {
            return Err(TicketNewError::EMPTY_DESCRIPTION);
        }
        if description.len() > 500 {
            return Err(TicketNewError::SIZE_DESCRIPTION);
        }
